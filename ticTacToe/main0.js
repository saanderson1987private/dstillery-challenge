class Grid {
  ...
  static getSquare(rowNum, colNum) {
    return document.getElementById(`${rowNum},${colNum}`);
  }

  static showsWinWith(targetSq, mark) {
    const rowNum = Number(targetSq.id[0]);
    const colNum = Number(targetSq.id[2]);
    const sq = Grid.getSquare;
    const hasCurMark = Grid.hasMark(mark);
...
class Player {
  constructor(mark, isComputer = false) {
    this.mark = mark;
  ...
  chooseSquare() {
    ...
    for (let i = 0; i < openSquares.length; i++) {
      if (Grid.showsWinWith(openSquares[i], this.mark)) square = openSquares[i];
    }
    const otherMark = this.mark === 'X' ? 'O' : 'X';
    for (let i = 0; i < openSquares.length; i++) {
      if (Grid.showsWinWith(openSquares[i], otherMark)) square = openSquares[i];
    }
    if (square === undefined) {
...
class Game {
  constructor (numPlayers) {
    ...
    const computerPlayer = numPlayers === 1 ? true : false;
    this.playerO = new Player('O', computerPlayer);
  ...
  takeTurn(square) {
    ...
    this.markCount ++;
    if (Grid.showsWinWith(square, this.currentPlayer.mark)) {
      this.declareWinner();
      return;
    }
